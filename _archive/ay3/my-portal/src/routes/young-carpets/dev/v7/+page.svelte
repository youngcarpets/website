<!--
	Young Carpets V7 — "Merged Improvements Preview"
	Exact main site with targeted improvements layered on:
	  1. Navbar: squared corners (16px, not pill)
	  2. Scroll progress indicator (2px bar at top)
	  3. Map: bigger on mobile and desktop
	  4. Contact: V6-style simplified (name + phone + email, no group labels)
	  5. Products: "Search by" filter bar above tiles, "Also" list removed
	  6. No "subcontract" language anywhere
	All product cards, SVG textures, animations, parallax — identical to main.
	2026-04-09
-->
<script lang="ts">
	...
	function showProduct(p: Product) { openProduct = p; }
	...
	// ── Device tilt parallax (same as main) ──
	...
	$effect(() => {
		...
		// iOS device-tilt parallax PARKED (same as main)
	...
	$effect(() => {
		...
		const onScroll = () => {
			...
			raf = requestAnimationFrame(() => {
				...
				// Clear section label near top of page
	...
	$effect(() => {
		if (typeof window === 'undefined') return;
		const sections = ['contact', 'map', 'about', 'what', 'products'];
	...
	function v7ScrollTo(id: string) {
		document.getElementById(id)?.scrollIntoView({ behavior: 'smooth' });
		mobileMenuOpen = false;
	}

	// ── V7: "Search by" — collapsible top-level ──
	let searchByOpen = $state(false);
	const searchCategories = [
		{ id: 'type', label: 'Type', desc: 'Carpet tile, LVT, hardwood, etc.' },
		{ id: 'purpose', label: 'Purpose', desc: 'What problem are you solving?' },
		{ id: 'building', label: 'Building Type', desc: 'Office, hospital, school, etc.' },
		{ id: 'supplier', label: 'Supplier', desc: 'Browse by manufacturer' },
		{ id: 'style', label: 'Style', desc: 'Look and feel preferences' },
	];
	let activeSearchCategory = $state('');

	// Products always show all for now — filtering comes later
	let filteredProducts = $derived(featureProducts
	);
</script>
...
<div class="yc-page" bind:this={pageEl}>
	{#if dev}
		<div class="yc-dev-version-badge" aria-hidden="true">v{ycVersion} · V7</div>
	{/if}

	<!-- V7: Scroll progress indicator -->
	<div class="v7-progress" style="transform: scaleX({scrollProgress})" aria-hidden="true"></div>

	<a class="yc-skip-link" href="#products">Skip to content</a>

	<!-- 3-layer architectural plan parallax backdrop (identical to main) -->
	...
	<!-- V7: Simplified navbar — YOUNG + active section + theme toggle + hamburger -->
	<nav class="v7-nav">
		<div class="v7-nav-left">
			<a href="/young-carpets/dev/v7" class="v7-nav-brand">YOUNG</a>
			{#if activeSection && sectionMap[activeSection]}
				<span class="v7-nav-section">{sectionMap[activeSection]}</span>
			{/if}
		</div>
		<div class="v7-nav-right">
			<ThemeToggle />
			<button
				class="v7-hamburger"
			...
			>
				<span class="v7-hamburger-bar" class:open={mobileMenuOpen}></span>
				<span class="v7-hamburger-bar" class:open={mobileMenuOpen}></span>
				<span class="v7-hamburger-bar" class:open={mobileMenuOpen}></span>
			</button>
		</div>
		{#if mobileMenuOpen}
			<div class="v7-mobile-menu">
				<button onclick={() => v7ScrollTo('products')}>Products</button>
				<button onclick={() => v7ScrollTo('what')}>Services</button>
				<button onclick={() => v7ScrollTo('about')}>About</button>
				<button onclick={() => v7ScrollTo('contact')}>Contact</button>
			</div>
		{/if}
	</nav>

	<!-- Hero (identical to main) -->
	<YcHero {motionNeedsTap} {motionGranted} onEnableMotion={enableMotion} />

	<!-- V7: "Search by" filters above product cards -->
	<section class="v7-search-by" use:reveal>
		<div class="yc-container">
			<button
				class="v7-search-toggle"
				onclick={() => searchByOpen = !searchByOpen}
				aria-expanded={searchByOpen}
			>
				<span class="v7-search-toggle-label">Search by</span>
				<span class="v7-search-toggle-chevron" class:open={searchByOpen}>{searchByOpen ? '−' : '+'}</span>
			</button>
			{#if searchByOpen}
				<div class="v7-search-dropdown">
					{#each searchCategories as cat}
						<button
							class="v7-search-option"
							class:v7-search-option--active={activeSearchCategory === cat.id}
							onclick={() => activeSearchCategory = activeSearchCategory === cat.id ? '' : cat.id}
						>
							<span class="v7-search-option-label">{cat.label}</span>
							<span class="v7-search-option-desc">{cat.desc}</span>
						</button>
					{/each}
					{#if activeSearchCategory}
						<p class="v7-search-coming-soon">Filters for "{searchCategories.find(c => c.id === activeSearchCategory)?.label}" coming soon.</p>
					{/if}
				</div>
			{/if}
		</div>
	...
	<!-- Products — real cards with SVG textures, filtered by Search By, NO "Also" list -->
	<section id="products" class="yc-section yc-section--products" use:reveal>
		<div class="yc-container">
			...
			<div class="yc-products-grid">
				{#each filteredProducts as p, i (p.code)}
					<button
	...
	<!-- Map (identical component — CSS override makes it bigger) -->
	<YcMapSection onOpen={openMap} />

	<!-- V7: Simplified contact (V6 gold standard — name + phone + email, no group label) -->
	<section id="contact" class="yc-section yc-section--contact v7-contact" use:reveal>
		<div class="yc-container">
			<p class="yc-section-eyebrow">Get In Touch</p>
			<h2 class="yc-section-title">Contact</h2>

			<div class="v7-contact-grid">
				<div class="v7-contact-people">
					{#each [
				...
				<div class="v7-contact-side">
					<div class="v7-contact-card">
						<h3>Accounting</h3>
						<div class="v7-person-card">
...
<style>
	/* ═══════════════ V7 TARGETED OVERRIDES ═══════════════ */

	/* 0. Tone down accent — tan-gold instead of bright yellow */
	:global(.yc-page) {
	...
	/* 0b. Square all pill shapes globally (eyebrows, badges, chips, buttons) */
	:global(.yc-page .yc-section-eyebrow),
	:global(.yc-page .yc-nav),
	:global(.yc-page .yc-nav-link),
	:global(.yc-page .yc-maple-badge),
	:global(.yc-page .yc-badge),
	:global(.yc-page .yc-chip),
	:global(.yc-page .yc-map-preview-pill),
	:global(.yc-page .yc-dev-version-badge),
	:global(.yc-page .yc-stat-number),
	:global(.yc-page .yc-team-row),
	:global(.yc-page .yc-modal-close),
	:global(.yc-page .yc-supplier-chip),
	:global(.yc-page .yc-modal-tab),
	:global(.yc-page .yc-modal-tab-btn),
	:global(.yc-page .yc-footer-col-title),
	:global(.yc-page .yc-back-link) {
		border-radius: 12px !important;
	}

	/* Footer YOUNG in Square721 */
	:global(.yc-page .yc-footer-brand-name) {
		font-family: 'Square721Nav', ui-serif, Georgia, serif !important;
	}
	/* Hide checkbox bullets in footer lists */
	:global(.yc-page .yc-footer-list li::before) {
		display: none !important;
	}
	:global(.yc-page .yc-footer-list li) {
	...
	/* 2. Navbar — simplified (YOUNG + toggle + hamburger) */
	@font-face {
	...
	.v7-nav {
		...
		box-shadow:
			...
			0 8px 24px var(--yc-shadow-45, rgba(0,0,0,0.45));
	}
	:global(:root[data-theme='light']) .v7-nav {
	...
	.v7-nav-section {
		...
		color: var(--yc-text-muted, rgba(200,200,200,0.5));
		margin-left: 0.75rem;
		white-space: nowrap;
		overflow: hidden;
		text-overflow: ellipsis;
		opacity: 0;
		animation: v7-section-fade-in 300ms ease forwards;
	}
	@keyframes v7-section-fade-in {
	...
	.v7-hamburger-bar.open:nth-child(1) { transform: rotate(45deg) translate(5px, 5px); }
	.v7-hamburger-bar.open:nth-child(2) { opacity: 0; }
	.v7-hamburger-bar.open:nth-child(3) { transform: rotate(-45deg) translate(5px, -5px); }
	.v7-mobile-menu {
		width: 100%;
		display: flex;
		flex-direction: column;
	...
	.v7-mobile-menu button {
		font-family: ui-monospace, 'SF Mono', monospace;
		font-size: 0.7rem;
		font-weight: 700;
		letter-spacing: 0.08em;
		text-transform: uppercase;
		color: var(--yc-text-muted, rgba(200,200,200,0.6));
		background: transparent;
		border: none;
	...
	.v7-search-toggle:hover {
		border-color: var(--yc-maple-gold, #d4b87a);
	}
	.v7-search-toggle-label {
		font-family: ui-monospace, 'SF Mono', monospace;
		font-size: 0.7rem;
		font-weight: 700;
		letter-spacing: 0.1em;
	...
	.v7-search-dropdown {
		...
		flex-wrap: wrap;
		gap: 0.5rem;
		animation: v7-section-fade-in 250ms ease forwards;
	}
	.v7-search-option {
		display: flex;
		flex-direction: column;
	...
	.v7-search-option:hover {
		border-color: var(--yc-maple-gold, #d4b87a);
	}
	.v7-search-option--active {
		border-color: var(--yc-maple-gold, #d4b87a);
		background: rgba(212, 184, 122, 0.08);
	}
	.v7-search-option-label {
		font-size: 0.85rem;
		font-weight: 600;
		color: var(--yc-text, #e6e6e8);
	}
	.v7-search-option-desc {
		font-size: 0.7rem;
		color: var(--yc-text-muted, rgba(200,200,200,0.5));
	}
	.v7-search-coming-soon {
		width: 100%;
		font-size: 0.75rem;
		color: var(--yc-text-muted, rgba(200,200,200,0.5));
		font-style: italic;
	...
	.v7-contact-card .v7-person-card {
		background: transparent;
		border: none;
	...
	.v7-contact-card .v7-person-card:last-child {
		border-bottom: none;
	}
</style>
